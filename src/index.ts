...
export const createWebShare: CreateWebShare = (options) => {
	...
	const platformName = Object.keys(platforms).find((name) =>
		platforms[name]?.uaRegex?.test(UA),
	)
		...
		async setShareInfo(info) {
			...
			if (!trueInfo(shareInfo)) {
				return await Promise.reject(
					new Error('必须传入有效的shareInfo，或者设置有效的globalShareInfo'),
				)
			}
			...
			if (platformConfig) {
				await loadJs(
					platformConfig.adapter.sdkSrc,
					platformConfig.adapter.isReady,
				)
